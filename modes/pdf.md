...
3. Extract 15-20 keywords from the JD (including technical terms and certifications).
4. Detect the JD language → CV language (EN default).
5. Detect the company location → paper format:
   - US/Canada → `letter`
   - Rest of the world → `a4`
6. Detect role archetype → adapt framing.
6b. Extract the exact job title (Target Role Title) from the job description.
...
10. Build the competency grid from JD requirements (12-16 keyword phrases).
11. Inject keywords naturally into existing achievements (NEVER invent/hallucinate).
...
16. Report: PDF path, page count, and keyword coverage percentage.

## ATS Rules (Clean Parsing)

- Single-column layout (no sidebars, no parallel columns).
- Exact job title from the JD in the main header as a subtitle (TARGET_ROLE).
- Standard headers: "Professional Summary", "Work Experience", "Education", "Skills", "Certifications", "Projects".
- No text in images/SVGs.
- No critical info in PDF headers/footers (ATS ignores them).
- UTF-8 selectable text (not rasterized/images).
- No nested tables.
- Abbreviations and full terms together (e.g., Natural Language Processing (NLP), AWS (Amazon Web Services)).
- JD keywords distributed: Summary (top 5), first bullet of each role, Skills section.
- Structured technical skills categorization (Languages, Frameworks/Libraries, DevOps/Cloud, Databases) in the Skills section.
...
1. Header (large name, subtitle with target role title from the JD, gradient, contact, portfolio link)
2. Professional Summary (3-4 lines, keyword-dense)
3. Core Competencies (12-16 keyword phrases in a flex-grid)
4. Work Experience (reverse chronological)
5. Projects (top 3-4 most relevant)
6. Education & Certifications
7. Skills (categorized: languages + technical)

## Keyword Injection Strategy (Ethical, Truth-based)

Examples of legitimate reformulations:
...
**NEVER add skills the candidate does not have. Only reformulate real experience using the exact vocabulary from the JD.**

## HTML Template

Use the template in `cv-template.html`. Replace placeholders `{{...}}` with custom content:
...
| `{{TARGET_ROLE}}` | Target job title extracted from the JD (e.g., Senior AI Engineer) |
...
| `{{SUMMARY_TEXT}}` | Customized professional summary rich in keywords |
| `{{SECTION_COMPETENCIES}}` | Core Competencies / Competencias Core |
| `{{COMPETENCIES}}` | `<span class="competency-tag">keyword</span>` × 12-16 |
| `{{SECTION_EXPERIENCE}}` | Work Experience / Experiencia Laboral |
| `{{EXPERIENCE}}` | HTML of each job with reordered bullets |
| `{{SECTION_PROJECTS}}` | Projects / Proyectos |
| `{{PROJECTS}}` | HTML of top 3-4 projects |
| `{{SECTION_EDUCATION}}` | Education / Formación |
| `{{EDUCATION}}` | HTML of education items |
| `{{SECTION_CERTIFICATIONS}}` | Certifications / Certificaciones |
| `{{CERTIFICATIONS}}` | HTML of certifications |
| `{{SECTION_SKILLS}}` | Skills / Competencias |
| `{{SKILLS}}` | HTML of skills (categorized in divs with skill-category) |
